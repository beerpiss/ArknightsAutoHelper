...
class RecordAddon(AddonBase):
    ...
    def replay_custom_record(
        self, record_name, mode=None, back_to_main=None, quiet=False
    ):
    ...
    def try_replay_record(self, record_name, quiet=False):
        try:
            ...
            self.logger.log(
                logging.DEBUG if quiet else logging.INFO, f"skip {record_name}, {e}"
            )
            return False
    ...
    def cli_record(self, argv):
        ...
        new_parser.add_argument(
            "--prefer-mode",
            choices=["match_template", "point"],
            help="回放模式：模板匹配（match_template）或固定坐标（point）",
        )
        new_parser.add_argument(
            "--back-to-main", action="store_true", help="是否从主界面开始回放"
        )
        new_parser.add_argument("--roi-size", type=int, help="模板匹配的大小")
        new_parser.add_argument(
            "--wait-seconds-after-touch", type=float, help="每次点击后的等待时间"
        )
        new_parser.add_argument("--description", help="操作记录的额外描述文本")
        new_parser.add_argument(
            "--threshold", type=float, help="模板匹配的阈值（0-1），越接近 1 代表相似度越高"
        )
        new_parser.add_argument("NAME", help="记录名称")

        play_parser = subparsers.add_parser("play", description="回放操作记录")
        play_parser.add_argument(
            "--mode", choices=["match_template", "point"], help="覆盖录制时选择的模式"
        )
        play_parser.add_argument(
            "--back-to-main",
