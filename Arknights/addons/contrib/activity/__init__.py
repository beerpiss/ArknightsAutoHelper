...
class ActivityAddOn(AddonBase):
    ...
    def run(self, target_stage_code, query_only=False):
        ...
        # print(rewards)
        stage_drops = [
            all_items_map[reward["id"]]["name"]
            for reward in rewards
            if reward["type"] == "MATERIAL" and reward["dropType"] == 2
        ]
        record_name = f'goto_{target_stage["stageType"]}_{target_stage["zoneId"]}'
        if query_only:
            return self.addon(RecordAddon).get_record_path(
                record_name
            ) or has_success_detect(target_stage["zoneId"])
        self.logger.info(
            f"{target_stage['code']}: {target_stage['name']}, 关卡掉落: {stage_drops}"
        )
        if has_success_detect(target_stage["zoneId"]) or not self.addon(
            RecordAddon
        ).get_record_path(record_name):
            pos = self.try_detect_and_enter_zone(target_stage)
            if pos != [-1, -1]:
                self.try_find_and_tap_stage_by_ocr(
                    target_stage["zoneId"], target_stage_code, stage_linear, pos
                )
    ...
    def create_record_for_activity(self, target_stage_code, record_name):
        ...
        self.addon(RecordAddon).create_custom_record(
            record_name,
            roi_size=32,
    ...
    def open_target_activity(self, target_stage):
        ...
        if self.check_current_activity(screen, activity_name):
            ...
            self.tap_point(
                box_center, self.delay_after_open_activity, randomness=(2, 2)
            )

    def check_current_activity(self, screen: Image, activity_name):
        vh, vw = self.vh, self.vw
        activity_rect = screen.crop(
            (12.083 * vh, 72.222 * vh, 54.167 * vh, 80.278 * vh)
        )
        ...
        for result in results:
            ...
            if score > max_score:
                ...
                max_text = text
        self.logger.info(
            f"check current activity, max text: {max_text}, score: {max_score}"
        )
    ...
    def try_find_and_tap_stage_by_ocr(
        self, zone_id, target_stage_code, stage_linear, pos
    ):
        try:
            self.logger.debug(
                f"target stage code: {target_stage_code}, stage linear: {stage_linear}"
            )
            self.addon(StageNavigator).find_and_tap_stage_by_ocr(
                None, target_stage_code, stage_linear
            )
