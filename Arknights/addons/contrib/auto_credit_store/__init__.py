...
from Arknights.addons.contrib.common_cache import (
    read_from_common_cache,
    save_to_common_cache,
)
...
credit_store_item_values_file = app.config_path.joinpath(
    "credit_store_item_values.yaml"
)
...
def get_credit_price(cv_screen, item_pos, ratio):
    ...
    cv2.rectangle(
        cv_screen.array,
        (x, y),
        (x + int(120 * ratio), y + int(28 * ratio)),
        (0, 255, 0),
    )
    # show_img(price_img)
...
class AutoCreditStoreAddOn(AddonBase):
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info(
            "信用商店价值数据从 credit_store_item_values.yaml 中加载, 其代表的是单位物品的理智价值, "
            "设定为负数则永远不会购买."
        )
    ...
    def get_credit(self):
        ...
        if common.check_get_item_popup(self.screenshot()):
            self.tap_rect(
                common.get_reward_popup_dismiss_rect(self.viewport), post_delay=2
            )
    ...
    def get_credit_from_friends(self):
        ...
        today = (
            datetime.now()
            .astimezone(tz=timezone(timedelta(hours=4)))
            .strftime("%Y-%m-%d")
        )
        if date == today:
    ...
    def get_total_credit(self, screen):
        ...
        rect = tuple(
            map(
                int,
                (100 * vw - 20.139 * vh, 3.333 * vh, 100 * vw - 2.361 * vh, 7.500 * vh),
            )
        )
    ...
    def calc_items(self, screen: Image):
        ...
        infos = inventory.get_all_item_details_in_screen(
            screen, exclude_item_ids={"other"}, only_normal_items=False
        )
        cv_screen = screen
        ...
        for info in infos:
            item_value = self.get_value(
                info["itemId"], info["itemName"], info["itemType"], info["quantity"]
            )
            ...
            self.log_text(
                f"[{info['itemId']}-{info['itemName']}] x {quantity}: {price}/{item_value}",
                DEBUG,
            )
        ...
        for i in range(len(solve_items)):
            if solve_items[i] == 1:
                ...
                cv2.rectangle(
                    cv_screen.array,
                    (pos[0] - int(64 * ratio), pos[1] - int(64 * ratio)),
                    (pos[0] + int(64 * ratio), pos[1] + int(64 * ratio)),
                    (255, 0, 255),
                    2,
                )
