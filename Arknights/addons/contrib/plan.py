...
class PlannerAddOn(AddonBase):
    ...
    def cli_arkplanner(self, argv):
        ...
        for i in range(len(materials)):
            ...
            print(
                "序号: %s, 材料等级: %s, 名称: %s"
                % (i, materials[i]["rarity"], materials[i]["name"])
            )
        required = {}
        while True:
            # s = '2/1'
        ...
        save_data = {"required": required, "owned": owned, "stages": stage_task_list}
    ...
    @custom_stage(
        ...
        ignore_count=True,
        title="执行刷图计划",
        description="Use Arkplanner to create a farming strategy",
    )
        ...
        if plan["stages"]:
            ...
            for task in plan["stages"]:
                ...
                if remain == 0:
                    continue
                self.logger.info(
                    "准备执行关卡 [%s], 计划次数: %s, 剩余次数: %s"
                    % (task["stage"], task["count"], remain)
                )
                update_flag = True
                c_id, remain = self.addon(StageNavigator).navigate_and_combat(
                    task["stage"], remain
                )
                task["remain"] = remain
                if remain > 0:
