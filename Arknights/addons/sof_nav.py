...
class ShipOfFoolsNavigator(AddonBase):
    ...
    def run(self, stage_code: str):
        ...
        while True:
            ...
            if match := self.match_roi(
                "maps/sof/banner", fixed_position=False, method="sift"
            ):
        ...
        section_rects = {
            1: match.bbox,
            2: self.load_roi("maps/sof/section2").bbox,
            3: self.load_roi("maps/sof/section3").bbox,
        }
        ...
        while True:
            ...
            tm = cv2.matchTemplate(
                screenshot.array,
                passed_icon.template.array,
                cv2.TM_SQDIFF_NORMED,
                mask=passed_icon.mask.array,
            )
            while True:
                ...
                x, y = min_loc
                stage_code_img = screenshot.crop(
                    Rect.from_xywh(
                        x,
                        y + passed_icon.template.height,
                        passed_icon.template.width,
                        23,
                    )
                ).convert("L")
                ...
                stage_code_ocr = ocr.acquire_engine_global_cached("en-us").recognize(
                    stage_code_img,
                    tessedit_pageseg_mode="13",
                    tessedit_char_whitelist="SNTR-0123456789",
                )
                self.richlogger.logtext(stage_code_ocr)
                cv2.rectangle(tm, (x - 4, y - 4), (x + 4, y + 4), max_val, -1)
                current_page_stages[
                    stage_code_ocr.text.replace(" ", "")
                ] = Rect.from_xywh(x, y - 80, passed_icon.template.width, 80).scale(
                    scale
                )
            ...
            if all(x > target_index for x in known_indices):
                self.logger.info(
                    "The level is to the left of the visible area, dragging to the right"
                )
                self.swipe_right()
            elif all(x < target_index for x in known_indices):
                self.logger.info(
                    "The level is to the right of the visible area, dragging to the left"
                )
                self.swipe_left()
            else:
    ...
    def swipe_left(self):
        self.control.input.touch_swipe(
            random.uniform(85, 95) * self.vw,
            random.uniform(40, 60) * self.vh,
            random.uniform(5, 15) * self.vw,
            random.uniform(40, 60) * self.vh,
            move_duration=0.5,
            hold_before_release=0.5,
        )
