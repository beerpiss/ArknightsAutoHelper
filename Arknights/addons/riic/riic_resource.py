...
portrait_mask_64_clipbox = Image.Rect.from_xywh(
    *cv2.boundingRect((portrait_mask_64.array[..., -1] == 255).astype(np.uint8))
)
portrait_mask_64_clipslice = (
    slice(portrait_mask_64_clipbox.y, portrait_mask_64_clipbox.bottom),
    slice(portrait_mask_64_clipbox.x, portrait_mask_64_clipbox.right),
)
...
def build_pack(repopath: Path):
    ...
    with open(
        repopath / "gamedata" / "excel" / "data_version.txt", "r", encoding="utf-8"
    ) as f:
    ...
    for file in glob.glob(str(repopath / "building_skill" / "*.png")):
        ...
        normal_blendf = (
            im.array[..., 0:3] * alpha[..., np.newaxis]
            + (1 - alpha[..., np.newaxis]) * background
        )
    ...
    normal_icons_stack = np.concatenate(
        [img[None, ...] for img in normal_icons]
    ).astype(np.float32)
    dark_icons_stack = np.concatenate([img[None, ...] for img in dark_icons]).astype(
        np.float32
    )
    ...
    for filename in all_portrait_files:
        ...
        select_img = (
            img.array[..., :3] * (1 - select_alpha) + select_alpha * select_tint
        )
        grayselect_img = np.dstack(
            (
                Image.fromarray(select_img.astype(np.uint8), "RGB").convert("L").array,
                img.array[..., -1],
            )
        )
        portrait_images.append(grayalpha_img)
    portrait_stack = np.concatenate([img[None, ...] for img in portrait_images]).astype(
        np.float32
    )
...
def refresh_pack():
    try:
        ...
        resp = session.get(
            "https://gh.cirno.xyz/github.com/ArknightsAutoHelper/ArknightsAutoHelper/releases/download/riic_pack/riic_pack.xz"
        )
...
def update_pack(filename):
    ...
    need_update = True
    try:
        current_pack = requests.get(
            "https://github.com/ArknightsAutoHelper/ArknightsAutoHelper/releases/download/riic_pack/riic_pack.xz"
        ).content
        current_version = (
            requests.get(
    ...
    if (repopath / ".git").is_dir():
        subprocess.run(
            ["git", "reset", "--hard", "origin/main"], check=True, cwd=repopath
        )
        subprocess.run(["git", "clean", "-fd"], check=True, cwd=repopath)
        subprocess.run(
            ["git", "pull", "--ff-only", "origin", "main"], check=True, cwd=repopath
        )
    else:
        subprocess.run(
            [
                ...
                "clone",
                "--depth=1",
                "https://github.com/yuanyan3060/Arknights-Bot-Resource",
                str(repopath),
            ],
            check=True,
        )
