...
def deblend(image, blend_color, blend_alpha, dtype=np.uint8):
    ...
    base = (blend - np.asarray(blend_color).astype(np.float32) * blend_alpha) / (
        1 - blend_alpha
    )
...
class RIICAddon(AddonBase):
    ...
    def refresh_cache(self):
        ...
        if self.skin_table is not new_skin_table:
            self.portrait_to_char = {
                x["portraitId"].lower(): x["charId"]
                for x in new_skin_table["charSkins"].values()
                if x.get("portraitId")
            }
    ...
    def check_in_riic(self, screenshot=None):
        if self.match_roi(
            "riic/overview", method="template_matching", screenshot=screenshot
        ):
            return True
        if roi := self.match_roi(
            "riic/pending", method="template_matching", screenshot=screenshot
        ):
    ...
    def enter_riic(self):
        ...
        if self.check_in_riic():
            return
        result = self.match_roi(
            "riic/riic_entry", fixed_position=False, method="sift", mode="L"
        )
        if result:
    ...
    def collect_all(self):
        ...
        while count < 2:
            if roi := self.match_roi(
                "riic/notification2", fixed_position=False, method="mse"
            ):
                while True:
                ...
                while roi := self.wait_for_roi(
                    "riic/collect_all2", timeout=2, fixed_position=False, method="mse"
                ):
    ...
    def recognize_layout(self):
        ...
        left_mask = imgops.scale_to_height(
            resources.load_image_cached("riic/layout.screen_mask.left.png", "L"),
            screenshot.height,
            Image.NEAREST,
        )
        right_mask = imgops.scale_to_height(
            resources.load_image_cached("riic/layout.screen_mask.right.png", "L"),
            screenshot.height,
            Image.NEAREST,
        )
        screen_mask = np.concatenate(
            [
                left_mask.array,
                np.full(
                    (
                        screenshot.height,
                        screenshot.width - left_mask.width - right_mask.width,
                    ),
                    255,
                    dtype=np.uint8,
                ),
        ...
        match = imgops.match_feature_orb(
            resources.load_image_cached("riic/layout.png", "L"),
            screenshot.convert("L"),
            templ_mask=templ_mask,
            haystack_mask=screen_mask,
            limited_transform=True,
        )
    ...
    def recognize_operator_select(
        self, full_recognize=False, skill_facility_hint=None
    ) -> list[OperatorBox]:
        ...
        img2find = np.concatenate(
            [
                scaled_screenshot.array[455:492, 605:],
                scaled_screenshot.array[876:913, 605:],
            ],
            axis=0,
        )
        dedup_set = set()
    ...
    def match_box_portrait(self, boximg: Image.Image, red_hint=False):
        from .riic_resource import (
            portrait_mask_64,
            portrait_names,
            portrait_maskclip_stack,
            portrait_mask_64_clipbox,
            portrait_mask_64_clip,
        )

        portrait2match = (
            boximg.subview(Rect.from_ltrb(5, 15, 183, 370))
            .resize(portrait_mask_64.size, Image.BILINEAR)
            .subview(portrait_mask_64_clipbox)
        )
        if red_hint:
            portrait2match = Image.fromarray(
                deblend(portrait2match.array, [100, 0, 0], 0.5), "RGB"
            )
            self.richlogger.logimage(portrait2match)
        mse_stack = batch_compare_mse_alpha(
            portrait2match.convert("L").array,
            portrait_mask_64_clip.array[..., 3],
            portrait_maskclip_stack,
        )
    ...
    def recognize_operator_box(
        self, img: Image.Image, full_recognize=False, face_hint=None
    ) -> OperatorBox:
        t00 = time.perf_counter()

        selected_check = np.mean(
            np.power(img.array[0:5].astype(np.float32) - [0, 152, 220], 2)
        )
        ...
        name_img = img.subview(
            (0, 375 * self.scale, img.width, img.height - 2 * self.scale)
        ).convert("L")
        ...
        if face_hint == "green":
            ...
            mood_img = (
                img.subview(Rect.from_xywh(44, 358, 127, 3).iscale(self.scale))
                .convert("L")
                .array
            )
        ...
        tagimg = img.subview(
            Rect.from_ltrb(35, 209, 155, 262).iscale(self.scale)
        ).resize(on_shift.size, Image.BILINEAR)
        tag = None
        ...
        if full_recognize:
            ...
            skill2_icon = img.subview(
                Rect.from_xywh(67, 285, 54, 54).iscale(self.scale)
            )
            ...
            has_room_check = img.subview(
                Rect.from_xywh(111, 9, 10, 4).iscale(self.scale)
            )
            mse = np.mean(
                np.power(
                    has_room_check.array.astype(np.float32) - has_room_check_color, 2
                )
            )
            self.richlogger.logtext(f"has_room_check mse={mse}")
            if mse < 200:
                room_img = img.subview(
                    Rect.from_xywh(42, 9, 74, 27).iscale(self.scale)
                ).array
                ...
                room = ocr_for_single_line(
                    room_img.convert("RGB").array, "0123456789FB"
                )
        ...
        result = OperatorBox(
            None, name, mood, tag, room, skill_icons=skill_icons, selected=selected
        )
    ...
    def enter_operator_selection(self, room="dorm1"):
        ...
        if self.match_roi("riic/clear_operator"):
            pass
        elif roi := self.match_roi(
            "riic/operator_button",
            method="mse",
            fixed_position=False,
            screenshot=screenshot.subview(
                (0, 0, screenshot.height * 0.18611, screenshot.height)
            ),
        ):
            self.tap_rect(roi.bbox)
    ...
    def iter_operator_list_pages(self, full_recognize=False):
        ...
        while True:
            ...
            self.control.input.touch_swipe(
                random.uniform(85, 90) * self.vw,
                random.uniform(40 * self.vh, 60 * self.vh),
                random.uniform(60, 65) * self.vh,
                random.uniform(40 * self.vh, 60 * self.vh),
                0.3,
                hold_before_release=0.3,
