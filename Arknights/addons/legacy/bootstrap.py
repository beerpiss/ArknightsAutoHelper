...
class BootstrapAddon:
    def check_game_active(self):  # 启动游戏 需要手动调用
        ...
        current = self.control.run_device_cmd(
    ...
    def module_login(self):
        ...
        self.logger.info(
            "发送坐标LOGIN_QUICK_LOGIN: {}".format(CLICK_LOCATION["LOGIN_QUICK_LOGIN"])
        )
        self.mouse_click(CLICK_LOCATION["LOGIN_QUICK_LOGIN"])
        self.delay(BIG_WAIT)
        self.logger.info(
            "发送坐标LOGIN_START_WAKEUP: {}".format(CLICK_LOCATION["LOGIN_START_WAKEUP"])
        )
