...
def main():
    ...
    while len(argv) > 0 and argv[0].startswith("-"):
        ...
        if opt == "--version-suffix":
            ...
            print(
                "usage: %s [--version-suffix SUFFIX] [--skip-zipball] [--] [PyInstaller extra args ...]"
                % sys.argv[0]
            )
            return
        elif opt == "--":
    ...
    with open(
        os.path.join(os.path.dirname(__file__), "..", "app", "release_info.py"), "w"
    ) as f:
        f.write("version = %r\n" % version)
    ...
    stash_id = (
        subprocess.run(
            ["git", "stash", "create", "stashed by makepackage"],
            capture_output=True,
            check=True,
        )
        .stdout.decode()
    ...
    subprocess.run(
        ["git", "archive", "-o", "resources.zip", stash_id or "HEAD", "resources"],
        check=True,
    )
    ...
    PyInstaller.__main__.run(
        [
            os.path.join(os.path.dirname(__file__), "..", "akhelper.spec"),
            "--noconfirm",
            *argv,
        ]
    )
