...
def build_app():
    ...
    git_files = [
        x.decode()
        for x in subprocess.check_output(["git", "ls-files", "-z"]).split(b"\0")
    ]
    ...
    py_compile.compile(
        os.path.join(build_dir, "release_info.py"),
        str(app_unpacked_dir / "app" / "release_info.pyc"),
    )
    ...
    shutil.copy(
        root / "config" / "logging.yaml", app_unpacked_dir / "config" / "logging.yaml"
    )
...
def build_runtime():
    ...
    shutil.copytree(
        root / "venv" / "Lib",
        runtime_dir / "Lib",
        ignore=lambda dir, files: [x for x in files if x.endswith(".pyc")]
        + ["__pycache__"],
    )
