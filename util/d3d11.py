...
class DXGI_SAMPLE_DESC(ctypes.Structure):
    _fields_ = [("Count", ctypes.wintypes.UINT), ("Quality", ctypes.wintypes.UINT)]
...
idldsl.define_winrt_com_method(
    ID3D11Device, "GetImmediateContext", retval=ID3D11DeviceContext, vtbl=40
)
# idldsl.define_winrt_com_method(ID3D11Texture2D, 'GetDevice', retval=ID3D11Texture2D, vtbl=3)
idldsl.define_winrt_com_method(
    ID3D11Texture2D, "GetDesc", retval=D3D11_TEXTURE2D_DESC, vtbl=10
)
idldsl.define_winrt_com_method(
    ID3D11DeviceContext,
    "Map",
...
idldsl.define_winrt_com_method(
    ID3D11DeviceContext, "Unmap", IUnknown, ctypes.c_uint, vtbl=15
)
idldsl.define_winrt_com_method(
    ID3D11DeviceContext, "CopyResource", IUnknown, IUnknown, vtbl=47
)
