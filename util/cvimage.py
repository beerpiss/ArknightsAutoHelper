...
def _get_valid_modes(shape, dtype):
    ...
    raise ValueError(
        f"cannot infer image mode from array shape {shape!r} and dtype {dtype!r}"
    )
...
class Image:
    ...
    def __init__(self, mat: np.ndarray, mode=None):
        ...
        if mode is None and len(valid_modes) > 1:
            warnings.warn(
                f"multiple mode inferred from array shape {mat.shape!r} and dtype {mat.dtype!r}: {' '.join(valid_modes)}, you might want to explicitly specify a mode"
            )
    ...
    def convert(
        ...
        dither=NotImplemented,
        palette=NotImplemented,
        colors=NotImplemented,
    ) -> Image:
        if (
            matrix is not NotImplemented
            or dither is not NotImplemented
            or palette is not NotImplemented
            or colors is not NotImplemented
        ):
        ...
        if target_pil_mode == self.mode:
            ...
            if target_cv_mode is None:
                if mode in pil_mode_mapping:
                    ...
                    raise NotImplementedError(
                        f"conversion from {self.mode} to {mode} not implemented yet"
                    )
    ...
    def rotate(
        ...
        if not (center or translate):
            ...
            if angle == 90 and expand:
                return Image(
                    cv2.rotate(self._mat, cv2.ROTATE_90_COUNTERCLOCKWISE), self.mode
                )
        ...
        newmat = cv2.warpAffine(
            self._mat,
            np.array(matrix).reshape(2, 3),
            (w, h),
            flags=resample,
    ...
    def resize(
        self, size, resample=None, box=NotImplemented, reducing_gap=NotImplemented
    ):
        if resample is None:
            if self.mode == "1":
                resample = NEAREST
        ...
        newmat = cv2.resize(
            self._mat,
            (int(round(size[0])), int(round(size[1]))),
    ...
    def show(self):
        ...
        multiprocessing.Process(
            target=_cvimage_imshow_helper.imshow, args=(title, native.array)
        ).start()
    ...
    def to_pil2(self, always_copy=False) -> tuple[PILImage.Image, bool]:
        ...
        if ystride < 0:
            ...
            ystep = -1
        fulllen = ystride * h
        contmat = np.lib.stride_tricks.as_strided(
            mat[::ystep, ...], shape=(fulllen,), strides=(1,)
        )
        return (
            PILImage.frombuffer(
                real_pil_mode, (w, h), contmat, "raw", pil_internal_mode, ystride, ystep
            ),
            mat is not oldmat,
        )
