...
class _ArrayInterfaceForPointer:
    ...
    def __init__(
        self, ptr, shape, typestr="|u8", strides=None, readonly=True, owner_object=None
    ):
...
def asarray(im: Image.Image, padding_channel="stride", allow_copy=True) -> np.ndarray:
    ...
    line_ptrs = np.asarray(
        _ArrayInterfaceForPointer(
            pilbuf["image"], (im.height,), _ptrtypestr, owner_object=imdata
        )
    )
    ...
    if np.all(pil_strides == pil_strides[0]):
        ...
        arr_strides = (
            (pil_strides[0], pixel_advance, 1)
            if pixel_advance > 1
            else (pil_strides[0], 1)
        )
        array_intf = {
            "version": 3,
            "shape": shape,
