...
class WorkerThread(threading.Thread):
    def __init__(
        ...
        self.allowed_calls = {
            ...
            "worker:set_refill_with_item": lambda x: self.helper.addon(
                "CombatAddon"
            ).configure_refill(with_item=bool(x))
            and None,
            "worker:set_refill_with_originium": lambda x: self.helper.addon(
                "CombatAddon"
            ).configure_refill(with_originium=bool(x))
            and None,
            "worker:set_max_refill_count": self.set_max_refill_count,
            "worker:module_battle": lambda stage, count: self.helper.addon(
                "StageNavigator"
            ).navigate_and_combat(stage, int(count))
            and None,
            "worker:module_battle_slim": lambda count: self.helper.addon(
                "CombatAddon"
            ).combat_on_current_stage(int(count))
            and None,
            "worker:clear_task": lambda: self.helper.addon("QuestAddon").clear_task()
            and None,
    ...
    def alert(self, title, text, level="info", details=None):
        ...
        self.output.put(
            dict(type="alert", title=title, message=text, level=level, details=details)
        )
