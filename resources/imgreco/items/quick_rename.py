...
def get_table(lang, table):
    return requests.get(f'https://raw.githubusercontent.com/Kengxxiao/ArknightsGameData/master/{lang}/gamedata/excel/{table}_table.json').text
...
for (_, val) in item_table_cn["items"].items():
    ...
    for path in ['archive', 'not-loot']:
        if os.path.exists(f"./{path}/{val['name']}.png"):
            os.rename(f"./{path}/{val['name']}.png", f"./{path}/{item_table_en['items'][item_id]['name']}.png")
