...
class Field(Generic[FieldType]):
    def __init__(
        self,
        type: Type[FieldType],
        default: Union[FieldType, UseDecoratedFunction] = _use_decorated_function,
        title: str = None,
        doc: str = None,
    ):
        self.type = type
...
class EnumField(Field[FieldType]):
    def __init__(
        self, values: Sequence[FieldType], default: FieldType, title=None, doc=None
    ):
    ...
    def __call__(self, subtype: Type[SubType]) -> SubType:
        if not issubclass(subtype, Schema):
            subtype = type(
                subtype.__name__,
                (Schema, subtype),
                {k: v for k, v in subtype.__dict__.items()},
            )
        self.type = subtype
        return self
...
class Schema:
    ...
    def __init__(
        self, store: Optional[Mapping] = None, parent: Optional[Schema] = None
    ):
        if store is None:
