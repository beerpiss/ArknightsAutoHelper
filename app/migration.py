...
def migrate_from_legacy(ydoc: Mapping):
    ...
    ydoc["stage_navigator"] = schemadef._generate_default_store(
        schema.root.stage_navigator
    )
    ydoc["grass_on_aog"] = schemadef._generate_default_store(schema.root.grass_on_aog)
    with contextlib.suppress(KeyError, AttributeError):
        if (
            isinstance(ydoc["device"]["extra_enumerators"], Mapping)
            and "bluestacks_hyperv" in ydoc["device"]["extra_enumerators"]
        ):
            ydoc["device"]["extra_enumerators"]["bluestacks_hyperv"] = True
    with contextlib.suppress(KeyError, AttributeError):
        if ydoc["behavior"].get("refill_ap_with_item", False) or ydoc["behavior"].get(
            "refill_ap_with_originium", False
        ):
            logger.warning(
                "Controlling automatic sanity restoration with behavior.refill_ap_with_item and behavior.refill_ap_with_originium is no longer supported, please enable it via the GUI or CLI before operation."
            )
    with contextlib.suppress(KeyError, AttributeError):
    ...
    with contextlib.suppress(KeyError, AttributeError):
        ...
        if isinstance(old_reporting, Mapping):
            ...
            ydoc["combat"]["penguin_stats"]["report_special_item"] = old_reporting[
                "report_special_item"
            ]
            del ydoc["reporting"]
    with contextlib.suppress(KeyError, AttributeError):
...
def migrate_from_1(ydoc: Mapping):
    ...
    with contextlib.suppress(KeyError, AttributeError):
        if ydoc["ocr"]["backend"] == "windows_media_ocr":
            logger.warn(
                "Windows OCR is disabled due to accuracy issues, settings changed to auto-select (only Tesseract is currently supported)."
            )
...
def migrate_from_4(ydoc: Mapping):
    ...
    # with contextlib.suppress(KeyError, AttributeError):
    ydoc["device"]["defaults"] = schemadef._generate_default_store(
        schema.root.device.type.defaults, 4
    )
    with contextlib.suppress(KeyError, AttributeError):
