...
def IAsyncOperation(T):
    ...
    define_winrt_com_method(
        cls, "put_Completed", propput=AsyncOperationCompletedHandler(T), vtbl=6
    )
    define_winrt_com_method(
        cls, "get_Completed", propget=AsyncOperationCompletedHandler(T), vtbl=7
    )
...
def IReference(T):
    cls = pinterface_type(
        "IReference", "61c17706-2d65-11e0-9ae8-d48564015472", (T,), (IInspectable,)
    )
    IAsyncOperation.known_types[(T,)] = cls
