    ...
    def touch_swipe(
        ...
        if ControllerCapabilities.LOW_LATENCY_INPUT not in caps:
            raise NotImplementedError(
                "default implementation of touch_swipe requires low-latency input"
            )
...
class ShellInputAdapter(InputProtocol):
    def __init__(self, controller: ADBController, displayid):
        ...
        if displayid is not None and displayid != 0:
            if controller.sdk_version >= 29:
                ...
                raise NotImplementedError(
                    "shell input on this device does not support multi display"
                )
        else:
        ...
        if controller.sdk_version >= 30:
            self.caps = (
                ControllerCapabilities.LOW_LATENCY_INPUT
                | ControllerCapabilities.TOUCH_EVENTS
            )
    ...
    def touch_tap(self, x, y, hold_time=0):
        if hold_time > 0:
            self.controller.adb.exec(
                f"{self.input_command} swipe {x} {y} {x} {y} {hold_time*1000:.0f}"
            )
        else:
    ...
    def touch_swipe(
        ...
        if self.support_motion_events:
            ...
            return _TouchEventsInputImpl.touch_swipe(
                self, x0, y0, x1, y1, move_duration, hold_before_release, interpolation
            )
        if hold_before_release > 0:
            warnings.warn(
        ...
        if interpolation != "linear":
            warnings.warn(
                "interpolation mode other than linear is not supported in shell mode"
            )
        self.controller.adb.exec(
            f"{self.input_command} swipe {x0} {y0} {x1} {y1} {move_duration*1000:.0f}"
        )
...
class ShellScreenshotAdapter(ScreenshotProtocol):
    def __init__(self, controller: ADBController, displayid):
        if controller.sdk_version <= 25:
            if displayid is not None and displayid != 0:
                raise NotImplementedError(
                    "shell screenshot on this device does not support multi display"
                )
        self.controller = controller
        ...
        if (
            use_transport is None
            and controller.device_info.slow_adb_connection
            and controller.device_info.emulator_hypervisor
        ):
        ...
        if pending_impl is not None:
            try:
                ...
                logger.debug(
                    "quirk implementation %s test passed", pending_impl.__name__
                )
            except:
                logger.debug(
                    "quirk implementation failed, falling back to adb raw",
                    exc_info=True,
                )
    ...
    def _screenshot_nc_listen(self):
        ...
        with self.controller.adb.exec_stream(
            f"screencap | nc -l -p {self._listen_port}"
        ):
...
class AahAgentClientAdapter(_TouchEventsInputImpl, ScreenshotProtocol):
        ...
        try:
            if self.controller.device_config.screenshot_transport != "adb":
                if loopback := self.controller.device_info.nat_to_host_loopback:
                    ...
                    future = rch.register_cookie()
                    logger.debug(
                        "listening for screenshot connection on %s:%d",
                        loopback,
                        rch.port,
                    )
                    self.client.open_screenshot(
                        "connect", (loopback, rch.port), future.cookie, future
                    )
    ...
    def get_input_capabilities(self) -> ControllerCapabilities:
        return (
            ControllerCapabilities.TOUCH_EVENTS
            | ControllerCapabilities.MULTITOUCH_EVENTS
            | ControllerCapabilities.LOW_LATENCY_INPUT
            | ControllerCapabilities.KEYBOARD_EVENTS
        )
...
class ADBController(Controller):
    def __init__(
        ...
        if (
            self.device_config.input_method == "aah-agent"
            or self.device_config.screenshot_method == "aah-agent"
        ):
            try:
                ...
                if self.device_config.screenshot_method == "aah-agent":
                    try:
                        ...
                        if self.device_info.emulator_hypervisor:
                            logger.warning(
                                "The emulator does not support aah-agent screenshots. If the emulator is freezing, change rendering mode in settings and restart the emulator, or turn off aah-agent screenshots in config.",
                                exc_info=True,
                            )
                        else:
                            logger.warning(
                                "The device does not support aah-agent screenshots. If the device is freezing, reboot the device and disable aah-agent screenshots in the config.",
                                exc_info=True,
                            )
                        self.device_config.save()
                    except:
                        logger.debug(
                            "failed to open aah-agent screenshot connection",
                            exc_info=True,
                        )
            except:
    ...
    def capabilities(self) -> ControllerCapabilities:
        return (
            self.input.get_input_capabilities()
            | self._screenshot_adapter.get_screenshot_capabilities()
        )
    ...
    def screenshot(self, cached: bool = True) -> cvimage.Image:
        ...
        if (
            not cached
            or self._last_screenshot is None
            or t0 > self._last_screenshot_expire
        ):
    ...
    def touch_swipe2(self, origin, movement, duration=None):
        ...
        logger.debug(
            "swiping: initial coordinates:({},{}); distance moved dX:{}, dy:{}".format(
                *origin, *movement
            )
        )
        if duration is None:
