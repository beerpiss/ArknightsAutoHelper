...
def _init():
    ...
    HcsOpenComputeSystem.argtypes = (
        ctypes.c_wchar_p,
        ctypes.POINTER(HCS_SYSTEM),
        ctypes.POINTER(ctypes.c_wchar_p),
    )
    ...
    def enum():
        ...
        with contextlib.suppress(Exception):
            ...
            for guidstr, obj in endpoints.items():
                ...
                if obj.get("VirtualNetworkName", None) not in (
                    "BluestacksNetwork",
                    "BluestacksNxt",
                ):
                    continue
                ...
                if policies := obj.get("Policies", None):
                    for policy in policies:
                        if policy.get("InternalPort", None) == 5555:
                            ...
                            vmname = [
                                x.get("Id")
                                for x in runningsystems
                                if GUID(x.get("RuntimeId")) == vmguid
                            ][0]
                            logger.debug(
                                "found bluestacks hyperv adb port %s for vm %s (%s)",
                                port,
                                vmguid,
                                vmname,
                            )
                            ...
                            devices.append(
                                ADBControllerTarget(
                                    ...
                                    2,
                                    1,
                                    override_identifier=f"hyperv:bstk:{vmname}",
                                    preload_device_info=preload,
                                )
                            )
        return devices
