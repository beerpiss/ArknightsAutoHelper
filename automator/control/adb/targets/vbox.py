...
def _init_win32():
    ...
    def enum():
        ...
        for vboxcls in vbox_clsids:
            ...
            if server:
                vbox_servers.append(
                    VBoxSerever(vboxcls.clsid, vboxcls.vendor, vboxcls.tag, server)
                )

        logger.debug("installed VirtualBox servers: %r", vbox_servers)

        running_processes = {
            pid: win32_process.resolve_image_name(pid)
            for pid in win32_process.all_pids()
        }
                                ...
                                if proto == "1" and guest_port == "5555":
                                    ...
                                    results.append(
                                        ADBControllerTarget(
                                            adb_server,
                                            None,
                                            f"{server.vendor}: {machine_name}",
                                            address,
                                            2,
                                            1,
                                            override_identifier=identifier,
                                            preload_device_info={
                                                "emulator_hypervisor": "vbox"
                                            },
                                        )
                                    )
            except:
                logger.debug(
                    "failed to check server %r, skipping", server, exc_info=True
                )
        return results
