...
class ADBDevice:
    def __init__(
        self, serial: Optional[str] = None, server: Optional[ADBServer] = None
    ):
        self.serial = serial
    ...
    def _create_session_retry(self, retry_count=0):
        ...
        try:
            ...
            if (
                retry_count == 0
                and e.args
                and isinstance(e.args[0], bytes)
                and b"not found" in e.args[0]
            ):
    ...
    def push(
        self, target_path: str, buffer: ReadableBuffer, mode=0o100755, mtime: int = None
    ):
        """push data to device"""
...
class ADBServer:
    ...
    def connect(self, device, timeout=None):
        resp = (
            self.service("host:connect:%s" % device, timeout=timeout)
            .read_response()
    ...
    def disconnect(self, device):
        resp = (
            self.service("host:disconnect:%s" % device)
            .read_response()
            .decode(errors="ignore")
        )
