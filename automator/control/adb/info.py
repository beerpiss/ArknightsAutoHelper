...
class ADBControllerDeviceInfo(Schema):
    def _probe_all(self):
        properties = [
            ...
            if isinstance(value, UserReadOnlyField)
        ]
    ...
    def adb_connection_speed(self):
        ...
        sock = controller.adb.exec_stream(
            "dd if=/dev/zero bs=2048 count=2048 status=none"
        )
        with sock:
        ...
        logger.debug(
            "ADB connection speed: %d bytes in %.3f s - %.2f MiB/s, TTFB %.3f s",
            rcvlen,
            tend - tfb,
            mbytes_per_sec,
            tfb - t0,
    ...
    def emulator_hypervisor(self):
        if (controller := self._get_controller()) is None:
            return 0.0
        if controller.adb.serial.startswith(
            "emulator-"
        ) or controller.adb.serial.startswith("127.0.0.1:"):
            board = controller.adb.exec("getprop ro.product.board")
            if b"goldfish" in board:
                return "avd"
            disk_vendor = (
                controller.adb.exec(
                    "cat /sys/class/block/?da/device/vendor 2>/dev/null"
                )
                .decode()
                .strip()
            )
            disk_model = (
                controller.adb.exec("cat /sys/class/block/?da/device/model 2>/dev/null")
                .decode()
                .strip()
            )
    ...
    def nat_to_host_loopback(self):
        ...
        if self.emulator_hypervisor == "vbox":
            ...
            possible_loopbacks = [
                x[: x.find(b" ")].decode() for x in arp.splitlines()[1:]
            ]
            if possible_loopbacks:
                loopback = _test_reverse_connection(
                    self._controller.adb, possible_loopbacks, self.nc_command
                )
                return loopback
        return None
    ...
    def host_l2_reachable(self):
        if self.emulator_hypervisor == "hyper-v":
            ...
            addrs = (
                self._controller.adb.exec('ip -4 addr | grep "scope global"')
                .decode()
                .strip()
                .split("\n")
            )
...
def _test_reverse_connection(
    device: ADBDevice, loopbacks: list[str], nc_command: str = "nc"
):
    if not loopbacks:
        return None
    ...
    for addr in loopbacks:
        ...
        cmd = "echo -n %sOKAY | %s -w 1 %s %d" % (
            future.cookie.decode(),
            nc_command,
            addr,
            rch.port,
        )
