...
class ShellNextFrontend:
    ...
    def delay(self, secs, allow_skip):
        ...
        fancywait.fancy_delay(
            secs, self.statusline, [skiphandler if allow_skip else skipdummy]
        )
...
def _connect_ident(args):
    ...
    targets = {
        x.override_identifier: x
        for x in enum_targets()
        if getattr(x, "override_identifier", None) is not None
    }
...
def debug(argv):
    ...
    def hook_instance(*args, **kwargs):
        ...
        instance.register_magic_function(
            command_internal, magic_kind="line", magic_name="akhelper"
        )
        return instance

    InteractiveShellEmbed.instance = hook_instance
    IPython.embed(
        banner1='use "%akhelper command" to access akhelper commands', colors="neutral"
    )
...
def _configure(prompt, helper_class):
    ...
    for name, record in _cli_registry.items():
        ...
        addon_cmds.append(
            (name, capture_value(record.owner, record.attr), record.get_help(helper))
        )
    global_cmds.extend(
        [
            *addon_cmds,
            ("interactive", interactive, interactive.__doc__),
            ("help", help, help.__doc__),
        ]
    )
    interactive_cmds.extend(
        [("connect", connect, connect.__doc__), *addon_cmds, ("exit", exit, "")]
    )
    if app.config.debug:
