...
def check_get_item_popup(img):
    ...
    icon1 = img.crop(
        (50 * vw - 6.389 * vh, 5.556 * vh, 50 * vw + 8.426 * vh, 18.981 * vh)
    ).convert("RGB")
...
def find_close_button(img):
    ...
    mtresult = cv2.matchTemplate(
        np.asarray(righttopimg), np.asarray(template), cv2.TM_CCOEFF_NORMED
    )
...
def check_dialog(img):
    ...
    img = (
        img.resize((1280, 720), Image.BILINEAR).convert("RGB").crop((0, 360, 1280, 640))
    )
    ...
    if max(coef1, coef2) > 0.5:
        return (
            ("yesno", (pt1[1] + 360) / 720 * oldheight)
            if coef1 > coef2
            else ("ok", (pt2[1] + 360) / 720 * oldheight)
        )
    return None, None
...
def recognize_dialog(img):
    ...
    return dlgtype, ocr.acquire_engine_global_cached("en-us").recognize(
        content, int(vh * 20), tessedit_pageseg_mode=11
    )
...
class RegionOfInterest:
    ...
    def with_target_viewport(self, width, height):
        ...
        vw = width / 100
        vh = height / 100
        left, top, right, bottom = np.asarray(
            self.bbox_matrix * np.matrix(np.matrix([[vw], [vh], [1]]))
        ).reshape(4)
...
class RoiMatchingMixin:
    ...
    def _ensure_roi(
        self, roidef: Union[str, RegionOfInterest], mode
    ) -> RegionOfInterest:
    ...
    def match_roi(
        self,
        roi: Union[str, RegionOfInterest],
        fixed_position: Optional[bool] = None,
        method: Literal["template_matching", "mse", "sift", None] = None,
        threshold=None,
        mode="RGB",
        screenshot=None,
        matching_mask=None,
        ...
        if fixed_position:
            ...
            if method == "mse":
                ...
                feature_result = imgops.match_feature(
                    template, compare, templ_mask=mask.array
                )
                ...
                raise RuntimeError(
                    "unsupported matching method %s for fixed-position ROI" % method
                )
            ...
            if method == "template_matching" or method == "ccoeff" or method == "mse":
                ...
                mask = (
                    roi.mask.resize(scaled_template.size)
                    if roi.mask is not None
                    else None
                )
                center, score = imgops.match_template(
                    screenshot,
                    scaled_template,
                    method=cv_method,
                    template_mask=mask.array,
                )
                point = (
                    center[0] - scaled_template.width / 2,
                    center[1] - scaled_template.height / 2,
                )
                result.score = score
                result.bbox = Image.Rect.from_xywh(
                    point[0], point[1], scaled_template.width, scaled_template.height
                )
            elif method == "sift":
                feature_result = imgops.match_feature(
                    roi.template,
                    screenshot,
                    templ_mask=roi.mask.array,
                    haystack_mask=matching_mask,
                )
                result.score = feature_result.matched_keypoint_count
                result.optimal_score = feature_result.template_keypooint_count
                result.threshold = min(feature_result.template_keypooint_count, 10)
                ...
                raise RuntimeError(
                    "unsupported matching method %s for non-fixed-position ROI" % method
                )
