...
def _parse_word(xmlword):
    attrs = dict(
        attrstr.split(" ", 1) for attrstr in xmlword.attrib["title"].split("; ")
    )
...
def _parse_line(xmlline):
    words = list(map(_parse_word, xmlline.findall('.//*[@class="ocrx_word"]')))
    attrs = dict(
        attrstr.split(" ", 1) for attrstr in xmlline.attrib["title"].split("; ")
    )
