...
def recognize(img):
    ...
    matches = [
        context.match_roi(f"before_operation/delegation_{variant}_{style}")
        for style in styles
    ...
    if style == "legacy":
        ...
        def stage_reco(img):
            ...
            return engine.recognize(
                img, tessedit_char_whitelist="SN-0123456789", tessedit_pageseg_mode="13"
            ).text.replace(" ", "")
...
def recognize_interlocking(img):
    ...
    consume_ap = imgops.compare_region_mse(
        img,
        (100 * vw - 31.944 * vh, 2.407 * vh, 100 * vw - 25.648 * vh, 8.426 * vh),
        "before_operation/interlocking/ap_icon.png",
        logger=logger,
    )

    apimg = img.crop(
        (100 * vw - 25.278 * vh, 2.407 * vh, 100 * vw - 10.093 * vh, 8.426 * vh)
    ).convert("L")
    reco = load_data()
    apimg = imgops.enhance_contrast(apimg, 80, 255)
    logger.logimage(apimg)
    ...
    delegated = imgops.compare_region_mse(
        img,
        (100 * vw - 32.963 * vh, 78.333 * vh, 100 * vw - 5.185 * vh, 84.167 * vh),
        "before_operation/interlocking/delegation_checked.png",
        logger=logger,
    )

    consumeimg = img.crop(
        (100 * vw - 11.944 * vh, 94.259 * vh, 100 * vw - 5.185 * vh, 97.500 * vh)
    ).convert("L")
    consumeimg = imgops.enhance_contrast(consumeimg, 80, 255)
    logger.logimage(consumeimg)
    consumetext, minscore = reco.recognize2(consumeimg, subset="-0123456789")
...
def check_ap_refill_type(img):
    ...
    item_icon = context.match_roi(
        "before_operation/refill_with_item_icon", method="ccoeff"
    )
    originium_icon = context.match_roi(
        "before_operation/refill_with_originium_icon", method="ccoeff"
    )
    ...
    icon1 = img.crop(
        (50 * vw + 25.972 * vh, 36.250 * vh, 50 * vw + 54.722 * vh, 61.250 * vh)
    ).convert("RGB")
