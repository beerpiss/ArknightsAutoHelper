...
def check_collectable_reward(img):
    ...
    if imgops.compare_region_mse(
        img,
        (50 * vw - 83.611 * vh, 20.093 * vh, 50 * vw - 77.407 * vh, 26.944 * vh),
        "task/collected_reward_set.png",
        logger=logger,
    ):
        # 第一个奖励显示已完成，即已领取全部奖励
        return False
    return imgops.compare_region_mse(
        img,
        (50 * vw + 70.278 * vh, 14.537 * vh, 50 * vw + 83.241 * vh, 22.778 * vh),
        "task/collect_all.png",
        logger=logger,
    )
...
def check_beginners_task(img):
    vw, vh = common.get_vwvh(img)
    icon1 = img.crop(
        (50 * vw - 24.028 * vh, 1.806 * vh, 50 * vw - 17.639 * vh, 7.917 * vh)
    ).convert("RGB")
