...
def detect_box(
    ...
    ppocr = get_ppocr()
    boxed_results = ppocr.detect_and_ocr(
        screen.array,
    ...
    for res in boxed_results:
        # print(res.ocr_text)
        cv2.drawContours(
            dbg_screen.array, [np.asarray(res.box, dtype=np.int32)], 0, (255, 0, 0), 2
        )
    ...
    cv2.drawContours(
        dbg_screen.array, [np.asarray(max_res.box, dtype=np.int32)], 0, (0, 255, 0), 2
    )
