...
class MaterialPlanning(object):
    def __init__(
        self, update=False, banned_stages={}, ConvertionDR=0.18, display_main_only=True
    ):
        """
    ...
    def get_item_id(self):
        ...
        item_name_to_id = {"id": {}, "zh": {}, "en": {}, "ja": {}, "ko": {}}

        additional_items = [
            {
                "itemId": "4001",
                "name_i18n": {"ko": "용문폐", "ja": "龍門幣", "en": "LMD", "zh": "龙门币"},
            },
            {
                "itemId": "0010",
                "name_i18n": {
        ...
        self.item_name_rv = {
            item_id_to_name[v]["zh"]: k for k, v in enumerate(item_array)
        }  # from (zh) name to idx
    ...
    def _pre_processing(self, material_probs):
        ...
        for server in servers:
            ...
            for stage in stages[server]:
                ...
                stage_code[server][stage_dct_rv[stage["stageId"]]] = stage["code_i18n"][
                    LanguageMap[server]
                ]
                for lang in languages:
                    stage_name_rv[lang][stage["code_i18n"][lang]] = stage_dct_rv[
                        stage["stageId"]
                    ]
        ...
        for drop in material_probs["matrix"]:
            try:
                self.probs_matrix[
                    self.stage_dct_rv[drop["stageId"]], self.item_dct_rv[drop["itemId"]]
                ] = drop["quantity"] / float(drop["times"])
            except:
        ...
        for rule in self.convertion_rules:
            ...
            for iname in outc_dct:
                convertion[self.item_name_rv[iname]] += (
                    outc_dct[iname] * self.ConvertionDR * outc_wgh[iname] / weight_sum
                )
    ...
    def update(
        ...
        if not force:  # if not force to update, try loading data from file.
            try:
                material_probs, self.convertion_rules, aog_stages = load_data(
                    path_stats, path_rules, path_aog_stages
                )
        ...
        if force:  # load from server.
            try:
                ...
                material_probs, self.convertion_rules, self.aog_stages = request_data(
        ...
        if filter_freq:
            ...
            for drop in material_probs["matrix"]:
                if (
                    drop["times"] >= filter_freq
                    and drop["stageId"] not in filter_stages
                ):
    ...
    def _get_plan_no_prioties(
        ...
        if convertion_dr != 0.18:
            convertion_outc_matrix = (
                convertion_outc_matrix - convertion_matrix
            ) / 0.18 * convertion_dr + convertion_matrix
        A_ub = (
            np.vstack([probs_matrix, convertion_outc_matrix])
            if outcome
            else np.vstack([probs_matrix, convertion_matrix])
        ).T
    ...
    def get_plan(
        ...
        if gold_demand:
            try:
                demand_lst[self.item_name_rv["LMD"]] = (
        ...
        def alive(stage):
            if (
                self.aog_stages
                and self.stage_code[server][self.stage_dct_rv[stage]]
                not in self.aog_stages
            ):
                return False
            if stage in exclude:
                return False
        ...
        for i, item in enumerate(self.item_array):
            if (
                y[i] >= 0
                and "作战记录" not in self.item_id_to_name[item]["zh"]
                and self.item_id_to_name[item]["zh"]
                not in ["龙门币", "赤金", "碳", "碳素", "碳素组", "经验", "家具"]
                and "技巧概要" not in self.item_id_to_name[item]["zh"]
            ):
                if y[i] > 0.1:
                    item_value = {
        ...
        gcost = -np.dot(
            x[len(cost_lst) :], convertion_matrix[:, self.item_name_rv["LMD"]]
        )
        gold = np.dot(n_looting, probs_matrix[:, self.item_name_rv["LMD"]])
        exp = (
            np.dot(n_looting, probs_matrix[:, self.item_name_rv["基础作战记录"]]) * 200
        ...
        for i, t in enumerate(n_looting):
            if t >= 0.1:
                ...
                if self.is_gold_or_exp(stage_name):
                    ...
                    exp -= t * (
                        probs_matrix[i, self.item_name_rv["基础作战记录"]] * 200
                ...
                if (
                    self.stage_code["CN"][self.stage_dct_rv[stage_name]][:2]
                    in ["SK", "AP", "CE", "LS", "PR"]
                    and self.display_main_only
                ):
                    continue
                target_items = np.where(probs_matrix[i] > 0.02)[0]
                items = {
                    self.item_id_to_name[self.item_array[idx]][output_lang]: float2str(
                        probs_matrix[i, idx] * t
                    )
                    for idx in target_items
                    if len(self.item_array[idx]) == 5
                    and self.item_array[idx] != "furni"
                }
                stage = {
        ...
        for i, t in enumerate(n_convertion):
            if t >= 0.1:
                ...
                if self.item_id_to_name[target_item]["zh"] in ["作战记录", "龙门币"]:
                    ...
                    continue
                materials = {
                    self.item_id_to_name[self.item_name_to_id["zh"][k]][
                        output_lang
                    ]: f"{v * t:.1f}"
                    for k, v in self.convertions_dct[
                        self.item_id_to_name[target_item]["zh"]
                    ].items()
                }
                synthesis = {
        ...
        if print_output:
            ...
            for stage in stages:
                ...
                print(
                    "Stage "
                    + self.stage_code[server][
                        self.stage_name_rv[output_lang][stage["stage"]]
                    ]
                    + "(%s times) ===> " % stage["count"]
                    + ", ".join(display_lst)
                )
    ...
    def update_convertion_processing(
        self, target_item: tuple, cost: int, source_item: dict, extraOutcome: dict
    ):
        """
        ...
        toAppend["costs"] = [
            {
                "count": x[1] / target_item[1],
                "id": self.item_name_rv[x[0]],
                "name": x[0],
            }
            for x in source_item.items()
        ]
        toAppend["extraOutcome"] = [
            {
                ...
                "id": self.item_name_rv[x[0]],
                "name": x[0],
                "weight": x[1] / target_item[1],
            }
            for x in Outcome.items()
        ]
        ...
        for i in range(1, 6):
            ...
            for item in t:
                if item["lowest_ap_stages"]:
                    aog_stages.update(
                        [x["code"] for x in item["lowest_ap_stages"]["normal"]]
                    )
                if item["balanced_stages"]:
                    aog_stages.update(
                        [x["code"] for x in item["balanced_stages"]["normal"]]
                    )
                if item["drop_rate_first_stages"]:
                    aog_stages.update(
                        [x["code"] for x in item["drop_rate_first_stages"]["normal"]]
                    )
