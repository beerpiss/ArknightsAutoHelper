...
class PenguinStatsReporter:
    ...
    def try_login(self, userid):
        ...
        try:
            ...
            resp = self.client.post(
                api_endpoint("/PenguinStats/api/v2/users"), data=str(userid)
            )
    ...
    def initialize(self):
        ...
        if app.version == "UNKNOWN":
            logger.warn(
                "Program version unavailable, please download the source code using git"
            )
    ...
    def set_penguin_data(self, stages: list[Stage], items: list[Item]):
        ...
        for i in items:
            if (
                i.get("itemType") == "RECRUIT_TAG"
                or not i["existence"][app.config.server]["exist"]
            ):
                continue
        ...
        unrecognized_items = (
            set(self.item_map.keys())
            - set(imgreco.itemdb.dnn_items_by_item_id.keys())
            - set(EXTRA_KNOWN_ITEMS)
        )
        extra_recognized_item_names = imgreco.itemdb.resources_known_items.keys()
        extra_recognized_item_ids = set(
            name_to_id_map.get(name, None) for name in extra_recognized_item_names
        )
        extra_recognized_item_ids.remove(None)
        unrecognized_items.difference_update(extra_recognized_item_ids)
        if unrecognized_items:
            logger.warn(
                "Unrecognized items were found in Penguin Statistics data: %s",
                ", ".join(unrecognized_items),
            )
            logger.warn("Reporting has been disabled to avoid statistical bias")
            self.noop = True
    ...
    def report(self, recoresult: EndOperationResult):
        ...
        if (
            sum(1 for drop in stage["dropInfos"] if drop.get("itemId", None) != "furni")
            == 0
        ):
            logger.info(
                "No drops for %s is available except furniture, not reporting", code
            )
            return ReportResult.NothingToReport
        ...
        flattenitems = [
            (groupname, item) for groupname, items in itemgroups for item in items
        ]

        try:
            flattenitems = list(
                event_preprocess(
                    recoresult.operation, flattenitems, exclude_from_validation
                )
            )
            report_special_item = app.config.combat.penguin_stats.report_special_item
            for item in flattenitems:
                if (
                    item[1].item_type == "special_report_item"
                    and not report_special_item
                ):
                    logger.error(
                        "Please go to Penguin Statistics to read reporting instructions, and change the "
        ...
        for itemdef in flattenitems:
            ...
            if groupname == "Lucky Drops":
                typeddrops.append(
                    ArkDrop(dropType="FURNITURE", itemId="furni", quantity=1)
                )
                continue
            ...
            if item.item_id in stage.get("recognitionOnly", []):
                logger.debug(
                    "Not reporting recognized items (recognitionOnly): %s", item.item_id
                )
                continue
            if (
                item.item_type == "special_report_item"
                and app.config.combat.penguin_stats.report_special_item
            ):
            ...
            if penguin_item is None:
                logger.warning(
                    "%s is not in the list of Penguin Statistics items", item.name
                )
                return ReportResult.NotReported
            ...
            if itemdef not in exclude_from_validation:
                filterresult = [
                    x
                    for x in dropinfos
                    if x.get("itemId", None) == itemid and x["dropType"] == droptype
                ]
                if filterresult:
                    dropinfo4item = filterresult[0]
                    if not _check_in_bound(dropinfo4item["bounds"], item.quantity):
                        logger.error(
                            "Item %s does not meet Penguin Statistics validation rules",
                            item,
                        )
                        return ReportResult.NotReported
                else:
                    logger.warning(
                        "Item is missing validation rules: %s: %r", groupname, item
                    )
            typeddrops.append(
                ArkDrop(dropType=droptype, itemId=itemid, quantity=item.quantity)
            )

        for groupinfo in dropinfos:
            if groupinfo.get("itemId", None) is None:
                kinds = sum(
                    1 for x in typeddrops if x["dropType"] == groupinfo["dropType"]
                )
                if not _check_in_bound(groupinfo["bounds"], kinds):
                    logger.error(
                        "The number of items (%d) in group %s does not meet Penguin Statistics validation rules",
                        kinds,
                        groupinfo["dropType"],
                    )
                    return ReportResult.NotReported
        ...
        try:
            ...
            resp = self.client.post(
                api_endpoint("/PenguinStats/api/v2/report"), json=req
            )
            resp.raise_for_status()
            if not self.logged_in:
